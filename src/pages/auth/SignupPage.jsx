...
function EmployeeJoinForm({ onSuccess }) {
  ...
  const handleStep1 = async (e) => {
    ...
    if (!orgCode || !firstName || !lastName || !email || !password)
      return setError('Please fill in all required fields.');
    if (!companyPreview)
      return setError('Invalid organization code. Please check with your Admin.');
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))
      return setError('Please enter a valid email address.');
    if (form.phone && detectIdentifierType(form.phone.trim()) !== 'phone')
      return setError('Please enter a valid 10-digit mobile number.');
      ...
      <div className="form-group">
        ...
        {companyPreview ? (
          ...
          <div style={{ marginTop: 6, fontSize: 12, color: 'var(--danger)' }}>
            <i className="fas fa-times-circle" /> Code not found. Check with your Admin.
          </div>
        ) : (
          <div className="form-hint">Ask your Admin for the organization code.</div>
        )}
      </div>
      ...
      <div className="auth-card">
        ...
        <div className="auth-title" style={{ marginBottom: 20 }}>
          {mode === 'admin' ? (
            ...
            <><h2>Join Your Company</h2><p>Enter the org code your Admin shared</p></>
          )}
        </div>
