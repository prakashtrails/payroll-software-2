...
function AdminSignupForm({ onSuccess }) {
  ...
  const handleSubmit = async (ev) => {
    ev.preventDefault();
    setError('');
    const { firstName, lastName, companyName, email, phone, password, confirmPassword } = form;
    ...
    // Start OTP verification — phone only
  ...
  const handleOtpVerified = async () => {
    ...
    try {
      ...
      onSuccess({ companyName: form.companyName, joinCode, needsEmailConfirm: !authData.session });
    } catch (err) {
    ...
    <>
      <form onSubmit={handleSubmit} noValidate>
        ...
        <div className="form-group">
          ...
          <input className="form-input" type="tel" placeholder="+919876543210" value={form.phone} onChange={set('phone')} />
          <div className="form-hint">Enter with country code (e.g. +91 for India)</div>
        </div>
        <div className="form-row">
          <div className="form-group">
...
function EmployeeJoinForm({ onSuccess }) {
  const [form, setForm] = useState({
    orgCode: '', firstName: '', lastName: '', email: '', phone: '',
    password: '', confirmPassword: '',
  });
  ...
  const handleSubmit = async (ev) => {
    ev.preventDefault();
    setError('');
    const { orgCode, firstName, lastName, email, phone, password, confirmPassword } = form;
    if (!orgCode || !firstName || !lastName || !email || !phone || !password)
      return setError('Please fill in all fields.');
    if (!companyPreview)
  ...
  const handleOtpVerified = async () => {
    ...
    try {
      ...
      await employeeJoinWorkspace(form.orgCode, form.firstName, form.lastName, authData.user.id);
