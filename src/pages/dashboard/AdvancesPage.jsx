...
export default function AdvancesPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const [advsRes, empsRes] = await Promise.all([
      supabase.from('advances').select('*').eq('tenant_id', tenant.id).order('created_at', { ascending: false }),
      supabase.from('profiles').select('id, first_name, last_name').eq('tenant_id', tenant.id).eq('status', 'Active'),
    ]);
  ...
  const saveAdvance = async () => {
    ...
    const emi = parseFloat(form.emi);

    if (editAdv) {
      const { error } = await supabase.from('advances').update({
        profile_id: form.profile_id, type: form.type, amount, emi,
        balance: amount - editAdv.paid, remarks: form.remarks,
      }).eq('id', editAdv.id);
      if (error) return showToast('Update failed', 'error');
      showToast('Advance updated', 'success');
    } else {
      const { error } = await supabase.from('advances').insert([{
        tenant_id: tenant.id, profile_id: form.profile_id, type: form.type,
        amount, emi, paid: 0, balance: amount, status: 'Active', remarks: form.remarks,
      }]);
  ...
  const getEmpName = (pid) => {
    const e = employees.find((x) => x.id === pid);
                ...
                <table>
                  ...
                  <tbody>
                    {filtered.length === 0 ? (
                      ...
                      const name = getEmpName(a.profile_id);
                        ...
                        <tr key={a.id}>
                          <td>
                            <div className="emp-cell">
                              ...
                              <div><div className="emp-name">{name}</div></div>
                            </div>
                          </td>
                          ...
                          <td>
                            ...
                            <button className="btn btn-outline btn-icon btn-sm" style={{ color: 'var(--danger)' }} onClick={() => deleteAdvance(a.id)}><i className="fas fa-trash" /></button>
                          </td>
                        </tr>
                      );
                    })}
      ...
      >
        ...
        <div className="form-row">
          <div className="form-group">
            ...
            <input className="form-input" type="number" value={form.amount} onChange={(e) => { setForm({ ...form, amount: e.target.value }); calcEMI(e.target.value, form.installments); }} />
          </div>
          <div className="form-group">
            <label className="form-label">Installments *</label>
            <input className="form-input" type="number" min="1" value={form.installments} onChange={(e) => { setForm({ ...form, installments: e.target.value }); calcEMI(form.amount, e.target.value); }} />
          </div>
        </div>
        <div className="form-group">
